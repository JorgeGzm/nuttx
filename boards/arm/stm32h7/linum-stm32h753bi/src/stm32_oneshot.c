                             ...
                             FAR void *arg)
{
  ...
  stm32_gpiowrite(GPIO_LED_RED, status_led);
...
int stm32_oneshot_init(int timer, uint16_t resolution)
{
  ...
  uint32_t sec;
  uint32_t nsec;
  uint32_t duration = USEC_PER_SEC;
...
/* turn off rgb leds */
stm32_gpiowrite(GPIO_LED_RED, 1);
stm32_gpiowrite(GPIO_LED_GREEN, 1);
stm32_gpiowrite(GPIO_LED_BLUE, 1);
